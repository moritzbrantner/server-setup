...
class FakeSystem:
    ...
    def _run(self, args: tuple[str, ...]) -> CommandResult:
        ...
        if args[:2] == ("timedatectl", "set-timezone"):
            ...
            self.files["/etc/timezone"] = args[2] + "\n"
            return CommandResult(0)
        ...
        if args[:2] == ("systemctl", "is-enabled"):
            return CommandResult(0 if args[-1] in self.services else 1)
        ...
        if args[:2] == ("ufw", "status"):
            lines = [f"Status: {'active' if self.ufw_active else 'inactive'}"]
            lines.extend(f"{rule} ALLOW Anywhere" for rule in sorted(self.ufw_rules))
            return CommandResult(0, "\n".join(lines) + "\n")
        if args[:2] == ("ufw", "allow"):
            self.ufw_rules.add(args[2])
            return CommandResult(0)
        if args[-1:] == ("enable",) and args[0] == "ufw":
            self.ufw_active = True
            return CommandResult(0)
        if args[-1:] == ("reload",) and args[0] == "ufw":
            return CommandResult(0)
        if args[0] == "ufw" and "default" in args:
            return CommandResult(0)
        if args[:2] == ("sshd", "-t"):
            return CommandResult(0) if self.sshd_valid else CommandResult(1, stderr="invalid sshd config")
        if args[:2] == ("sshd", "-T"):
            return CommandResult(0, "".join(f"port {port}\n" for port in sorted(self.sshd_ports)))
        if args[0] == "ss":
            text = "\n".join(f"LISTEN 0 128 0.0.0.0:{port} 0.0.0.0:*" for port in sorted(self.ports))
            return CommandResult(0, text)
        if args[0] == "docker":
            if not self.docker_available:
                return CommandResult(127)
            if args[1:4] == ("service", "inspect", "dokploy"):
                if not self.dokploy_installed:
                    return CommandResult(1)
                if "--format" in args:
                    version = self.dokploy_version or "latest"
                    return CommandResult(0, f"dokploy/dokploy:{version}@sha256:test\n")
                return CommandResult(0, "{}")
            if args[1:3] == ("ps", "--format"):
                return CommandResult(0, "dokploy.1.test\n" if self.dokploy_installed else "")
            if args[1:3] == ("info", "--format"):
                return CommandResult(0, "active\n" if self.swarm_active else "inactive\n")
            if args[1:3] == ("network", "inspect"):
                return CommandResult(0 if self.network_exists else 1)
        if args[0] == "curl":
            url = next((value for value in args if value.startswith("https://")), "")
            match = re.search(r"/releases/download/([^/]+)/install\.sh", url)
            self.download_version = match.group(1) if match else None
            return CommandResult(0)
        if args[0] == "bash" and args[1:2] == ("/tmp/server-setup-dokploy-install.sh",):
            self.docker_available = True
            self.commands.add("docker")
            self.dokploy_installed = True
            self.dokploy_version = self.download_version
            self.swarm_active = True
            self.network_exists = True
            self.ports.update({80, 443, 3000})
            return CommandResult(0)
        if args[0] == "df":
